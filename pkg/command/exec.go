...
func CombinedOutput(command string, output bool, options ...string) (string, error) { // nolint:revive // output is a valid flag parameter
	...
	out, err := exec.CommandContext(ctx, command, options...).CombinedOutput() //nolint:gosec // G204: intentional - this function is designed to execute arbitrary commands // nosemgrep: go.lang.security.audit.dangerous-exec-command.dangerous-exec-command
...
func CombinedOutputStr(command string, output bool) (string, error) { // nolint:revive // output is a valid flag parameter
	ctx := context.Background()
	out, err := exec.CommandContext(ctx, "sh", "-c", command).CombinedOutput() //nolint:gosec // G204: intentional - this function is designed to execute arbitrary shell commands // nosemgrep: go.lang.security.audit.dangerous-exec-command.dangerous-exec-command
	if output {
		log.Println(command)
		log.Println(string(out))
	}
