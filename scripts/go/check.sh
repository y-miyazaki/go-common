...
function run_go_fmt {
    ...
    if [[ "$FIX_MODE" == "true" ]]; then
        ...
        # Check if formatting is needed by using gofmt -l for checking without modification
        local fmt_output
        fmt_output=$(gofmt -l $(go list -f {{.Dir}} "$TARGET_PATTERN") 2>/dev/null | grep '\.go$' || true)
