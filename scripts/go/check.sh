...
function run_go_fmt {
    ...
    if [[ "$FIX_MODE" == "true" ]]; then
        ...
        # Check if formatting is needed by using gofmt -l to list files needing formatting
        local fmt_output
        fmt_output=$(gofmt -l $(find . -name "*.go" -not -path "./vendor/*" | head -100) 2>/dev/null || true)
